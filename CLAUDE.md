...
Project state is kept in four committed files **under `docs/`** so it travels with the repo:

- **`docs/status.md`** — log of actions undertaken (newest first). Append a dated entry at the **end** of any session that changed state.
- **`docs/next_steps.md`** — the current plan / what to do next. Keep it the single source of truth for "what now"; rewrite it as priorities change.
- **`docs/lessons_learned.md`** — decisions and knowledge we don't want to lose. Append when a decision is made or a fact is established; only remove an entry if it turns out wrong.
- **`docs/project_protocols.md`** — the project corpus: the single definitive list of protocols used for the analysis, split into chosen and rejected. Add any new protocol here when it's brought into the project.
